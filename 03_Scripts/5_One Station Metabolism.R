...
onestation<-onestation %>%rename('GPP_1'='GPPavg', 'ER_1'='ER', 'NEP_1'='NEP')
onestation<-onestation[,c('Date', 'ID', 'GPP_1', 'ER_1', 'NEP_1')]

twostation<- read_csv("04_Outputs/master_metabolizer_parsed.csv")
twostation<-twostation %>%rename('GPP_2'='GPPavg', 'ER_2'='ER', 'NEP_2'='NEP')
...
ggplot(master, aes(Date)) +
  ...
  geom_line(aes(y=depth,color='h'))+
 facet_wrap(~ ID, ncol=2)


ggplot(master, aes(Date)) +
  geom_line(aes(y=GPP,color='mean'))+
  geom_line(aes(y=GPP_1,color='GPP1'))+
