...
stream <- reduce(data, full_join, by = c("ID", 'Date'))%>%
  ...
  group_by(ID, day)%>%
  mutate(Temp=if_else(is.na(Temp), mean(Temp, na.rm=T), Temp))

GasDome <- function(gas,stream) {
  
  gas<-gas %>% mutate(day=as.Date(Date), hour=hour(Date))
  gas<-left_join(gas,stream, by=c('hour', 'day','ID'), relationship = "many-to-many")
  ...
  gas<-gas %>%
    mutate(pCO2_water=CO2_enviro/1000000, day=as.Date(Date),
           pCO2_air=max(gas$CO2, na.rm=T)/1000000, sec=second(Date))%>%
    mutate( sec_cumulative = as.numeric(difftime(Date, first(Date), units = "secs")))
  ...
  gas<-gas%>% select(day,ID,rep,CO2,CO2_enviro,slope, depth,k600_1.day)
