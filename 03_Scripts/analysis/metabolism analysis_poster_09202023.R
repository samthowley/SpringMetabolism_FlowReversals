...
slopey<-(expression(atop(paste("Metablic Flux "/ Delta~"h"), paste('g'~O[2]/m^3/'day'))))
...
###Scatter plots#######
theme_scatter<-theme()+  theme(axis.text.x = element_text(size = 18, angle=0),
...
(IU_sc<-ggplot(data=IU, aes(x=stage_diff)) + 
    geom_point(aes(y=GPPavg), size=1, color='darkgreen', alpha=0.2)+
    geom_point(aes(y=ER*-1), size=1, color='darkred', alpha=0.2)+
    geom_point(aes(y=NEP), size=1, color='blue', alpha=0.2)+
    ...
    geom_smooth(aes(x=stage_diff, y=NEP), color='blue', size=0.75,
                data=IU, se = FALSE, method='lm')+
    xlab(" ")+ggtitle("IU")+ylab(flux)+
    scale_x_continuous(n.breaks=3) +
    scale_y_continuous(n.breaks=3)+theme_scatter)
...
(ID_sc<-ggplot(data=ID, aes(x=stage_diff)) + 
    ...
    geom_point(aes(y=ER*-1), size=1, color='darkred', alpha=0.2)+
    geom_point(aes(y=NEP), size=1, color='blue', alpha=0.2)+
    ylab(" ")+scale_color_manual(values='black')+
    geom_smooth(aes(x=stage_diff, y=GPPavg), color='darkgreen', size=0.75,
...
quantile(springs$ER, probs = c(0,0.25,0.5,0.75,1), na.rm=T)

boxplot_theme<-theme()+theme(axis.text.x = element_blank(),
      axis.text.y = element_text(size = 24, angle=0),
      axis.title.y =element_blank(),
      axis.title.x =element_blank(),
      plot.title = element_text(size = 24),
...
(ER<-ggplot(springs, aes(x=site, y=ER)) + 
  ...
  ylab(flux)+
  ggtitle(ERflux)+
  scale_y_continuous( breaks=c(0, -13, -33))+boxplot_theme+
  stat_summary(fun=mean, colour="white", geom="point", 
               size=1, show.legend=FALSE))

springs<-filter(springs, GPPavg<40)
quantile(springs$GPPavg, probs = c(0,0.25,0.5,0.75,1), na.rm=T)

(GPP<-ggplot(springs, aes(x=site, y=GPPavg)) + 
  geom_boxplot(outlier.colour="black", outlier.size=1,fill="darkgreen")+
  ggtitle(GPPflux)+
  scale_y_continuous( breaks=c(0, 6 , 40))+boxplot_theme+
  stat_summary(fun=mean, colour="white", geom="point", 
               size=1, show.legend=FALSE)+ theme_sam)

quantile(springs$NEP, probs = c(0,0.25,0.5,0.75,1), na.rm=T)

(NEP<-ggplot(springs, aes(x=site, y=NEP)) + 
  geom_boxplot(outlier.colour="black", outlier.size=1,fill="blue")+
  ylab(flux)+
  ggtitle(NEPflux)+
  scale_y_continuous( breaks=c(-27, -7 , 30))+boxplot_theme+
  stat_summary(fun=mean, colour="white", geom="point", 
               size=1, show.legend=FALSE))
...
together<-plot_grid(boxplots,scatter,  nrow=2, rel_heights = c(3/5,1.9/5))
