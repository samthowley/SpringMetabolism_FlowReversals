...
IchRR$stage_avg<-IchRR$stage_avg/h_prior
...
(d02<-ggplot(IchRR, aes(x=disturb_count))+
  geom_line(aes(y=NEP, color="NEP"), size=0.8)+
  geom_vline(xintercept=-2)+  geom_vline(xintercept=57))

Ichrecov<-filter(IchRR,disturb_count>-2 & disturb_count<57)
modNEP<-lm(NEPmean ~ disturb_count, data = Ichrecov)
cf <- coef(modNEP)
(InterceptNEP <- cf[1])
(SlopeNEP <- cf[2])
...
ggplot(IchRR, aes(x=disturb_count))+
  geom_line(aes(y=stage_avg, color="NEP"), size=0.8)+
  geom_vline(xintercept=-0)+  geom_vline(xintercept=14)
...
Ichrecov<-filter(IchRR,disturb_count>0 & disturb_count<14)
modNEP<-lm(NEPmean ~ disturb_count, data = Ichrecov)
cf <- coef(modNEP)
(InterceptNEP <- cf[1])
(SlopeNEP <- cf[2])
...
title<-expression(paste(recovery[metabolic]/recovery[stage]))
...
(f<-ggplot(recovery, aes(stage, shape=site))+
    ...
    ggtitle("GPP and ER Recovery", subtitle = "by Stage")+
    xlab(h)+
...
(g<-ggplot(recovery, aes(u, shape=site))+
    ...
    xlab(u)+scale_y_continuous(trans='log10')+
    ylab(title)+
    theme(axis.text.x = element_text(size = 19, angle=0),
          axis.text.y = element_text(size = 19, angle=0),
...
summary(lm(stage ~ ratio_ER, data = recovery))
summary(lm(stage ~ ratio_GPP, data = recovery))
