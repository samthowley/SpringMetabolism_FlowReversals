...
OtBO<- Otter %>% mutate(RI = case_when(
  Date> "2022-08-01" & Date<"2022-11-10"~ 2))
...
Otrecov<-filter(OtBO,disturb_count>0 & disturb_count<17)
...
Otrecov<-filter(OtBO,disturb_count>-3 & disturb_count<17)
...
Otrecov<-filter(OtBO,disturb_count>20 & disturb_count<31)
...
Otrecov<-filter(OtBO,disturb_count>4 & disturb_count<38)
...
OtBO<- Otter %>% mutate(RI = case_when(
  Date> "2023-01-24" & Date<"2023-5-01"~ 2))
OtBO<-filter(OtBO, RI== "2")
...
max(OtBO$stage_avg, na.rm=T)
OtBO$disturb_count<-OtBO$consec-35
...
ggplot(OtBO, aes(x=disturb_count))+
  geom_line(aes(y=stage_avg), size=0.8)+
  geom_vline(xintercept=0)+  geom_vline(xintercept=55)


x<-filter(OtBO, disturb_count>=-4& disturb_count <=7)
...
Otrecov<-filter(OtBO,disturb_count>0 & disturb_count<20)
modNEP<-lm(NEPmean ~ disturb_count, data = Otrecov)
cf <- coef(modNEP)
(InterceptNEP <- cf[1])
(SlopeNEP <- cf[2])
...
Otrecov<-filter(OtBO,disturb_count>10 & disturb_count<60)
modGPP<-lm(GPPmean ~ disturb_count, data = Otrecov)
cf <- coef(modGPP)
(InterceptGPP<- cf[1])
(SlopeGPP <- cf[2])
...
Otrecov<-filter(OtBO,disturb_count>0 & disturb_count<55)
modH<-lm(stage_avg ~ disturb_count, data = Otrecov)
cf <- coef(modH)
(InterceptH<- cf[1])
(SlopeH <- cf[2])
...
plot_grid(d08+ggtitle(" Otter: 08/2022"),d02+ggtitle("Otter: 02/2022"))
####AllenMill##########
...
ggplot(AllenMill, aes(x=Date))+
  geom_line(aes(y=NEP, color="K1d"), size=0.8)
...
AMBO<- AllenMill %>% mutate(RI = case_when(
  Date> "2022-08-10" & Date<"2022-10-30"~ 2))
...
AMBO$disturb_count<-AMBO$consec-36
...
(d08<-ggplot(AMBO, aes(x=disturb_count))+
  ...
  geom_vline(xintercept=0)+  geom_vline(xintercept=13))



AMrecov<-filter(AMBO,disturb_count>0 & disturb_count<7)
...
AMFR<- AllenMill %>% mutate(RI = case_when(
  Date> "2023-01-11" & Date<"2023-04-12"~ 2))
...
AMrecov<-filter(AMFR,disturb_count>0 & disturb_count<19)
modNEP<-lm(NEPmean ~ disturb_count, data = AMrecov)
cf <- coef(modNEP)
(InterceptNEP <- cf[1])
(SlopeNEP <- cf[2])
...
AMBO<- AllenMill %>% mutate(RI = case_when(
  Date> "2023-06-01" & Date<"2023-08-11"~ 2))
AMBO<-filter(AMBO, RI== "2")
...
plot_grid(d08+ggtitle("AM: 08/2022"),d02+ggtitle("AM: 02/2023"),
          d06+ggtitle("AM: 06/2023"))
...
ggplot(LF, aes(x=Date))+
  geom_line(aes(y=GPPavg, color="met"), size=0.8)+
...
LFFR<- LF %>%mutate(RI = case_when(
  Date> "2023-01-18" & Date<"2023-04-18"~ 2))
...
LFFR$disturb_count<-LFFR$consec-36
...
LFrecov<-filter(LFFR,disturb_count>0 & disturb_count<24)
...
LFrecov<-filter(LFFR,disturb_count>-3 & disturb_count<38)
...
LFrecov<-filter(LFFR,disturb_count>0 & disturb_count<54)
...
LFRR<- LF %>% mutate(RI = case_when(
  Date> "2023-05-21" & Date<"2023-08-21"~ 2))
...
LFRR$disturb_count<-LFRR$consec-42
...
d06<-ggplot(LFRR, aes(x=disturb_count))+
  geom_line(aes(y=NEP, color="NEP"), size=0.8)+
  geom_vline(xintercept=0)+  geom_vline(xintercept=29)

LFrecov<-filter(LFRR,disturb_count>0 & disturb_count<13)
modNEP<-lm(NEPmean ~ disturb_count, data = LFrecov)
cf <- coef(modNEP)
(InterceptNEP <- cf[1])
(SlopeNEP <- cf[2])
...
LFrecov<-filter(LFRR,disturb_count>0 & disturb_count<28)
modGPP<-lm(GPPmean ~ disturb_count, data = LFrecov)
cf <- coef(modGPP)
(InterceptGPP<- cf[1])
(SlopeGPP <- cf[2])
...
LFrecov<-filter(LFRR,disturb_count>0 & disturb_count<23)
modH<-lm(stage_avg ~ disturb_count, data = LFrecov)
cf <- coef(modH)
(InterceptH<- cf[1])
(SlopeH <- cf[2])
...
plot_grid(d02+ggtitle("LF: 02/2023"),
          d06+ggtitle("FL: 06/2023"))
...
IchRR<- Ich %>% mutate(RI = case_when(
  Date> "2022-08-06" & Date<"2022-10-31"~ 2))
...
IchRR$disturb_count<-IchRR$consec-38
...
Ichrecov<-filter(IchRR,disturb_count>0 & disturb_count<38)
...
Ichrecov<-filter(IchRR,disturb_count>0 & disturb_count<19)
...
Ichrecov<-filter(IchRR,disturb_count>3 & disturb_count<28)
...
IchRR<- Ich %>% mutate(RI = case_when(
  Date> "2023-01-20" & Date<"2023-05-11"~ 2))
IchRR<-filter(IchRR, RI== "2")
...
max(IchRR$stage_avg, na.rm=T)
IchRR$disturb_count<-IchRR$consec-34
...
d02<-ggplot(IchRR, aes(x=disturb_count))+
  geom_line(aes(y=NEP, color="NEP"), size=0.8)+
  geom_vline(xintercept=0)+  geom_vline(xintercept=64)

Ichrecov<-filter(IchRR,disturb_count>-2 & disturb_count<59)
modNEP<-lm(NEPmean ~ disturb_count, data = Ichrecov)
cf <- coef(modNEP)
(InterceptNEP <- cf[1])
(SlopeNEP <- cf[2])
...
Ichrecov<-filter(IchRR,disturb_count>10 & disturb_count<59)
modER<-lm(ERmean ~ disturb_count, data = Ichrecov)
cf <- coef(modER)
(InterceptER<- cf[1])
(SlopeER <- cf[2])
...
Ichrecov<-filter(IchRR,disturb_count>0 & disturb_count<59)
Ichrecov$stage_RR<- Ichrecov$stage_avg/mean(Ich_low$stage_avg, na.rm=T)
modH<-lm(stage_avg ~ disturb_count, data = Ichrecov)
cf <- coef(modH)
(InterceptH<- cf[1])
...
IchRR<- Ich %>% mutate(RI = case_when(
  Date> "2023-06-07" & Date<"2023-08-11"~ 2))
IchRR<-filter(IchRR, RI== "2")
...
max(IchRR$stage_avg, na.rm=T)
IchRR$disturb_count<-IchRR$consec-24

ggplot(IchRR, aes(x=disturb_count))+
  geom_line(aes(y=stage_avg))
names(u)
u<- IchRR %>%
  mutate( ERmean= rollapply(ER,4,mean, fill=NA, partial=TRUE, align='left'))
u<-u[,c(32,1)]
...
Ichrecov<-filter(IchRR,disturb_count>-3 & disturb_count<12)
modNEP<-lm(NEPmean ~ disturb_count, data = Ichrecov)
cf <- coef(modNEP)
(InterceptNEP <- cf[1])
(SlopeNEP <- cf[2])
...
Ichrecov<-filter(IchRR,disturb_count>0 & disturb_count<15)
modER<-lm(ERmean ~ disturb_count, data = Ichrecov)
cf <- coef(modER)
(InterceptER<- cf[1])
(SlopeER <- cf[2])
...
Ichrecov<-filter(IchRR,disturb_count>-20 & disturb_count<0)
modGPP<-lm(GPPmean ~ disturb_count, data = Ichrecov)
cf <- coef(modGPP)
(InterceptGPP<- cf[1])
(SlopeGPP <- cf[2])
...
Ichrecov<-filter(IchRR,disturb_count>0 & disturb_count<12)
modH<-lm(stage_avg ~ disturb_count, data = Ichrecov)
cf <- coef(modH)
(InterceptH<- cf[1])
(SlopeH <- cf[2])
...
R_R<-rbind(US27,Ichetucknee, LF, AM, Otter)

R_R$NEP_recovery<-abs(R_R$NEP_recovery)
R_R$GPP_recovery<-abs(R_R$GPP_recovery)
R_R$ER_recovery<-abs(R_R$ER_recovery)

R_R$NEP_recovery[R_R$NEP_recovery>92] <- NA
R_R$GPP_recovery[R_R$GPP_recovery>92] <- NA
R_R$ER_recovery[R_R$ER_recovery>92] <- NA
R_R$stage_recovery[R_R$stage_recovery>92] <- NA


R_R$ratio_GPP<- (R_R$GPP_recovery/R_R$stage_recovery)
R_R$ratio_ER<- (R_R$ER_recovery/R_R$stage_recovery)
R_R$ratio<- (R_R$NEP_recovery/R_R$NEP_recovery)


(R_R$ratio_GPPlog<- log(R_R$ratio_GPP))
(R_R$ratio_ERlog<- log(R_R$ratio_ER))
...
recovery <- read_excel("Z:/SpringsProject_Sam&Paul/Master/recovery.xlsx")
title<-expression(paste('log'(recovery[metabolic]/recovery[stage])))
h<-expression(paste( h[actual]-h[minimum]))
u<-"velocity m/s"

recovery$a<-'a'
(f<-ggplot(recovery, aes(stage, shape=site))+
        geom_smooth(aes(x=stage, y=ratio_ERlog, group=a), color='darkred', size=0.75,
                data=recovery, se = FALSE, method='lm')+
    geom_point(aes(y=ratio_ERlog), size=5,color="darkred")+
    geom_point(aes(y=ratio_GPPlog), size=5, color='darkgreen' )+
    geom_hline(yintercept = 1, linetype='dashed')+
    ggtitle("GPP and ER Recovery Post- Inverted Floods")+
    xlab(h)+
    ylab(title)+
...
(g<-ggplot(recovery, aes(u, shape=site))+
    
    geom_smooth(aes(x=u, y=ratio_GPPlog, group=a), color='darkgreen', size=0.75,
                data=recovery, se = FALSE, method='lm')+
    geom_smooth(aes(x=u, y=ratio_ERlog, group=a), color='darkred', size=0.75,
                data=recovery, se = FALSE, method='lm')+
