...
check_ratios <- function(siteBO) {
  x<-c("Date","DO","depth","ER","GPPavg",'depth_diff')
  siteBO<-siteBO[,x]
...
recovery_calc <- function(siterecov) {
  ...
  GPPkeep<-c("count","GPPavg_ratio")
  GPPrecov<-siterecov[,GPPkeep]
  ind <- which.min(GPPrecov$GPPavg_ratio)
  GPPrecov<-GPPrecov[-c(0:ind),]
  GPPrecov<-GPPrecov %>% filter(GPPavg_ratio<3)
  modGPP<-lm(GPPavg_ratio ~ count, data = GPPrecov)
  cf <- coef(modGPP)
  (GPPrecov_results<-(1-cf[1])/cf[2])

  ggplot(GPPrecov, aes(x=count,y=GPPavg_ratio))+
...
##### data####
master <- read_csv("02_Clean_data/master_met4_notparsed.csv")
master<-master[,c("Date","DO" ,"GPPavg","ER","depth","ID")]
...
AMFR<- AM %>% mutate(RI = case_when(Date> "2023-10-01" & Date<"2024-01-15"~ 2))
AMFR<-filter(AMFR, RI==2)
#ggplot(AMFR, aes(Date))+geom_line(aes(y=GPPavg), size=1)
AMFR_1223<-check_ratios(AMFR)

AMFR_1223<-filter(AMFR_1223, count<95)
ggplot(AMFR_1223, aes(x=count,y=hmean))+
  geom_point(size=1)+geom_smooth(method = "lm")
...
#ggplot(LFFR_0822, aes(count))+geom_point(aes(y=hmean))
(LFFR_0822<-recovery_calc_FR(LFFR_0822))
...
#ggplot(LFFR_0323, aes(count))+geom_point(aes(y=hmean))
(LFFR_0323<-recovery_calc(LFFR_0323))

LFFR<- LF %>% mutate(RI = case_when(
  Date> "2023-05-25" & Date<"2023-08-26"~ 2))
LFFR<-filter(LFFR, RI==2)
#ggplot(LFFR, aes(Date))+geom_line(aes(y=depth))
...
IUFR<-filter(IUFR, RI==2)
#ggplot(IUFR, aes(x=Date))+geom_line(aes(y=GPPavg), size=1)
...
recov$GPP_recov<-as.numeric(recov$GPP_recov)
