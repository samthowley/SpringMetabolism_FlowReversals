...
ggplot(GB, aes(Date)) + geom_line(aes(y=GPPavg))+
...
GBFR<- GB %>% mutate(RI = case_when(
  Date> "2022-08-11" & Date<"2022-12-27"~ 2))
GBFR<-filter(GBFR, ER> -30)
GB_0622<-extract_reduce(GB, GBFR)

GBFR<- GB %>% mutate(RI = case_when(
  Date> "2023-12-18" & Date<"2024-05-11"~ 2))
...
OtFR<- OS %>% mutate(RI = case_when(
  Date> "2022-07-15" & Date<"2022-10-20"~ 2))
OS_0722<-extract_reduce(OS, OtFR)

OtFR<- OS %>% mutate(RI = case_when(
  Date> "2023-05-15" & Date<"2023-08-20"~ 2))
...
AMFR<- AM %>% mutate(RI = case_when(
  Date> "2023-01-07" & Date<"2023-05-06"~ 2))
...
AMFR<- AM %>% mutate(RI = case_when(
  Date> "2023-11-12" & Date<"2024-01-11"~ 2))
...
LFFR<- LF %>% mutate(RI = case_when(
  Date> "2023-12-30" & Date<"2024-08-01"~ 2))
